...
import { Search, Download, FileSpreadsheet, Table as TableIcon } from 'lucide-react';
...
export const DataExport: React.FC = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((row) => {
      ...
      if (reconciledFilter !== 'all' && row.is_reconciled !== reconciledFilter) return false;
      if (!q) return true;
      return Object.values(row).some((v) =>
        v !== null && v !== undefined && String(v).toLowerCase().includes(q)
      );
    });
  }, [data, searchQuery, typeFilter, reconciledFilter]);
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="py-16 flex items-center justify-center text-slate-500 text-xs">
